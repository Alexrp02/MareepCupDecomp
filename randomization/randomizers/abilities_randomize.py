...
for i in range(1, 10):
    ...
    with open(
        f"../src/data/pokemon/species_info/gen_{i}.h", "r", encoding="utf-8"
    ) as file:
    ...
    for line in lines:
        ...
        if line.startswith("        .abilities = {"):
            ...
            abilities_list = [
                ability.strip() for ability in abilities_string.split(",")
            ]
    ...
    with open(
        f"../src/data/pokemon/species_info/gen_{i}.h", "w", encoding="utf-8"
    ) as file:
